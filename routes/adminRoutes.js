...
// Cria cupom promocional fresco (nunca usado)
...
// Repõe estoque de todos os pneus ativos para 50 unidades
router.post('/estoque/reset', exigirAdmin, async (req, res) => {
  try {
    await pool.query(`UPDATE pneus SET estoque = 50 WHERE ativo = TRUE`);
    res.json({ mensagem: 'Estoque reposto para 50 unidades em todos os pneus ativos.' });
  } catch (err) {
    res.status(500).json({ erro: err.message });
  }
});
